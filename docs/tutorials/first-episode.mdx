...
<Tip>
Episode IDs use the format `ep_<ULID>`.
</Tip>
...
```bash
ls .noesis/episodes/ep_01JH6Z2V9Q2K6Y6N0QZ7K2QW8C/
```
...
```bash
jq . .noesis/episodes/ep_01JH6Z2V9Q2K6Y6N0QZ7K2QW8C/summary.json
```
...
```bash
jq -s . .noesis/episodes/ep_01JH6Z2V9Q2K6Y6N0QZ7K2QW8C/events.jsonl
```
...
Every episode emits these phases into the trace (faculties can exist even when they emit nothing):
...
<Steps>
  ...
  <Step title="Intuition">
    Records policy guidance (hints/interventions/veto signals).
  </Step>
  ...
  <Step title="Direction">
    Applies plan directives before governance.
  </Step>
  <Step title="Governance">
    Audits the plan and can veto before action execution.
  </Step>
  <Step title="Act">
  ...
  <Step title="Terminate">
    Records the episode end status and message.
  </Step>
  <Step title="Insight">
    Computes KPI rollups from the trace.
  </Step>
  <Step title="Memory">
    Records persistence/port outcomes.
  </Step>
</Steps>
