...
- Default enforce veto (`governance_pause_on_veto=false`): `direction(status="blocked") → action_candidate → governance → terminate` (no `act`), and the run is sealed (`final.json` + `manifest.json`).
- Pause-on-veto (`governance_pause_on_veto=true`): `direction(status="blocked") → action_candidate → governance → run.interrupt → run.checkpoint` (no `act`, no `terminate`), and the run remains unsealed until continuation.
