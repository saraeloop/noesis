...
Pause intent before a checkpoint or manual review gate.
...
Optional caller-provided explanation for the pause.
</ResponseField>

### `run.checkpoint`

Append-only checkpoint anchor for same-run continuation.
...
```json
{
  ...
  "payload": {
    "kind": "run.checkpoint",
    "status": "paused",
    "checkpoint_id": "chk_abc123def456",
    "event_offset": 42,
    "checkpoint_path": "checkpoints/chk_abc123def456/checkpoint.json"
  },
  "caused_by": "evt_interrupt123"
}
```
...
Stable checkpoint identifier (`chk_*`).
</ResponseField>

<ResponseField name="payload.event_offset" type="number" required>
1-indexed event count at checkpoint creation.
</ResponseField>

<ResponseField name="payload.checkpoint_path" type="string" required>
Relative path to checkpoint metadata file.
</ResponseField>

Checkpoint metadata lives at `checkpoints/<checkpoint_id>/checkpoint.json`
and includes `event_offset`, `last_event_id`, `state_hash`,
`artifact_manifest_hash`, and optional `adapter_label`.

### `run.resume`

Lifecycle evidence that continuation has been authorized on the same run ID.
...
<ResponseField name="payload.event_offset" type="number" required>
Checkpoint anchor offset used for consistency validation.
</ResponseField>

<ResponseField name="payload.resume_strategy" type="string" required>
Currently always `same_run_id`.
</ResponseField>

Before emitting `run.resume`, runtime validates checkpoint consistency against
the current trace and artifacts (`event_offset`/`last_event_id`, `state_hash`,
and `artifact_manifest_hash`).
