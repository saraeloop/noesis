...
Candidate identity linked from governance/action-candidate flow (when present).
</ResponseField>

<ResponseField name="payload.step_id" type="string">
Plan step ID associated with the action (when available).
</ResponseField>

<ResponseField name="payload.provenance" type="object">
Structured provenance (source, evidence IDs, policy/adapter metadata).
</ResponseField>

<ResponseField name="payload.result_artifacts" type="array">
Artifacts produced by the action (type/uri/hash plus optional `x-` fields).
</ResponseField>
