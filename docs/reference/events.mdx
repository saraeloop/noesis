...
Episode identifier for the run that owns this trace record.
</ResponseField>
...
Faculty marker automatically added for faculty phases: `intuition`, `direction`, `governance`, or `insight`.
</ResponseField>
...
<ResponseField name="evidence_ids" type="array" required>
Evidence references carried forward into the event record. Use an empty array when no evidence applies.
</ResponseField>
...
Runtime writers validate the persisted record, not just the caller input:

- `write_event()` adds `id` when omitted, but every stored event has an `id`.
- `event.timestamp` must be an ISO 8601 string. When `metrics.completed_at` is present, the runtime aligns `event.timestamp` to that value.
- Event timestamps cannot move backward. Non-metric events are clamped to the prior timestamp; metric-bearing events older than the prior event fail validation.
- `metrics`, when present, must include `started_at`, `completed_at`, and numeric `duration_ms`.
- Faculty phases (`intuition`, `direction`, `governance`, `insight`) receive a matching top-level `faculty` field.
- Runtime lifecycle records use `phase="runtime"`, require `event_type`, and require `payload.status`.

Common validation failures:

| Symptom | Likely cause |
| --- | --- |
| `event missing required keys` | Missing `timestamp`, `episode_id`, `phase`, `payload`, or `evidence_ids` |
| `event.payload must be a dict` | Payload was serialized as a string/list instead of an object |
| `event.metrics is missing ...` | Metrics object omitted `started_at`, `completed_at`, or `duration_ms` |
| `runtime events require event_type` | A lifecycle/projection event used `phase="runtime"` without a subtype |
