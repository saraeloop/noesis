...
Optional process label used to group related episodes in the process registry.
</ParamField>
...
<ParamField body="process" type="str | None" default="None">
Optional process label used to group related adapter-backed episodes in the
process registry.
</ParamField>
...
episode_id = ns.solve("process this", using="my.module:adapter_fn", process="etl")
```
...
When an episode is started with `process=...`, each row also includes a `process`
block from the episode summary:
...
The `id` is derived from the resolved workspace identity plus the process name.
Use the stable `name` for operator-facing filters and the opaque `id` for exact
machine references.
