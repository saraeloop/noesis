...
- Checkpoints store `event_offset`, `last_event_id`, `state_hash`, `artifact_manifest_hash`, and adapter metadata.
...
- If `manifest.json` exists when the checkpoint is created, `artifact_manifest_hash` is the SHA-256 of that manifest.
- If the run is paused before `manifest.json` exists, Noēsis hashes the checkpointed artifact set (`events.jsonl`, `prompts.jsonl`, `state.json`, `summary.json`, `learn.jsonl` when present).
- For pre-manifest checkpoints, only the `events.jsonl` prefix up to `event_offset` is included so later `run.checkpoint` and `run.resume` events do not invalidate the checkpoint.
- Resume fails closed if the manifest appears, disappears, changes, or if checkpointed artifacts change before continuation.
...
- `CheckpointConsistencyError`: checkpoint anchor (`event_offset`, `last_event_id`, `state_hash`, `artifact_manifest_hash`) no longer matches artifacts.
