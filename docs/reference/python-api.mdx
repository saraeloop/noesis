...
Resolve canonical artifact file paths for an episode.

```python
from noesis.io import paths

artifact_paths = paths("ep_01JH6Z2V9Q2K6Y6N0QZ7K2QW8C")
print(artifact_paths["events"])
print(artifact_paths["summary"])
print(artifact_paths["manifest"])
```

```python
from noesis.io import paths

artifact_paths = paths(episode_id: str, *, context: Any | None = None) -> dict[str, str]
```
...
<Note>
Use `ns.get()` to read config. `ns.config()` is not part of the public module API.
</Note>

### Projection diagnostics helpers (advanced)

Use these helpers when validating that persisted `state.json` remains derivable from `events.jsonl`:
...
plan_projection = project_plan_state(events)
assert plan_projection is not None
assert plan_projection.to_dict() == state["plan"]

state_projection = project_state_projection(events)
assert state_projection is not None
assert state_projection.status == state["outcomes"]["status"]
assert state_projection.links == state["links"]
```
