...
Dataclass for learning signals. Required fields are `proposal_id`, `kind`, and
`target`. Optional fields include `policy_id`, `policy_version`, rationale and
evidence, score/confidence, status, metadata, acceptance state, and
`revert_handle`.
...
`learn.jsonl` empty (created) or absent.
</Note>
