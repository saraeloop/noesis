...
Workspace root to snapshot for verification. Also used as the workspace identity seed for process registration.
</ParamField>

<ParamField body="process" type="str | None" default="None">
Optional process name. Groups this run under a stable process identity (`sha256(workspace|name)[:12]`). Omit to auto-derive a display name from the workspace path.
</ParamField>
...
episode_id = ns.run("Draft release notes", process="release-bot")
```
...
<ParamField body="process" type="str | None" default="None">
Optional process name. Same identity rules as `ns.run(..., process=...)`.
</ParamField>
...
`ns.checkpoint(...)` returns a `checkpoint/1.0.0` pointer written to `checkpoints/<checkpoint_id>/checkpoint.json`:

| Field | Role |
| --- | --- |
| `event_offset` | Number of `events.jsonl` records at checkpoint time |
| `last_event_id` | Causal parent event id |
| `state_hash` | SHA-256 of `state.json` |
| `artifact_manifest_hash` | Digest of the current artifact set |
| `adapter_label` | Persisted adapter identity (when available) |

`artifact_manifest_hash` is `sha256(manifest.json)` when that file exists. On paused/unsealed runs it usually does not, so the runtime digests the current artifact set instead (`events.jsonl`, `state.json`, `summary.json`, plus optional `prompts.jsonl` / `learn.jsonl`). The `events.jsonl` contribution is the prefix of `event_offset` lines, so later `run.checkpoint` / `run.resume` events do not invalidate the pointer.

Resume re-computes those anchors. It fails closed if:

- `events.jsonl` history no longer matches `event_offset` / `last_event_id`
- `state.json` hash changed
- `manifest.json` appeared, disappeared, or changed since the checkpoint
- `caused_by` is supplied and is neither `last_event_id` nor the latest event id
...
- `CheckpointConsistencyError`: checkpoint anchors no longer match (`event_offset`, `last_event_id`, `state_hash`, or `artifact_manifest_hash`).
...
| --- | --- | --- |
| `CheckpointConsistencyError: ... artifact manifest hash ...` | `manifest.json` was added, removed, or edited after checkpoint | Restore the artifact set that existed at checkpoint time, or start a new run |
| `CheckpointConsistencyError: ... state hash ...` | `state.json` changed on disk | Do not edit `state.json` between checkpoint and resume |
| `CheckpointConsistencyError: ... causal anchor ...` | `events.jsonl` ids/order changed | Restore `events.jsonl`; the log is fail-closed and append-only |
| `RunSealedError` | `final.json` already exists | Resume is only valid on unsealed (paused) runs |
