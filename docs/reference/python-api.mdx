...
Human process label for grouping runs. Stored in the process registry under `.noesis/processes/`; does **not** nest episode directories. Identity is `sha256(workspace|name)[:12]`.
</ParamField>
...
episode_id = ns.run("Apply canary", process="rollout", seed=42)
```
...
# 1) Enable pause-on-veto in enforce mode so a veto pauses instead of sealing
ns.set(
    governance_mode="enforce",
    governance_pause_on_veto=True,
)

# 2) Start a run. If governance vetoes, solve returns an unsealed episode
#    with run.interrupt + run.checkpoint already on the trace.
...
if checkpoint_event is None:
    # Run completed and sealed. ns.interrupt(...) would raise RunSealedError.
    pass
else:
    checkpoint_id = checkpoint_event["payload"]["checkpoint_id"]
    # 3a) Emit resume evidence only (no execution continuation)
    resume_event_id = ns.resume(episode_id, checkpoint_id=checkpoint_id)
    # 3b) Continue execution from checkpoint on the same run ID
    episode_id = ns.resume_run(
        episode_id,
...
Manual `ns.interrupt` / `ns.checkpoint` is only valid on **unsealed** runs. After `final.json` exists, use a new run rather than mutating the sealed episode.
...
Each row includes `episode_id`, `task`, `started_at`, `flags`, `success`, `process`, `manifest`, and `manifest_status` (when `strict_manifest=True`). There is no `timestamp` field; use `started_at`.
...
The runtime writes this index best-effort after sealing (`noesis/core.py`, `ttl_days=30`). `vacuum()` drops expired **index rows** only; it does not delete `.noesis/episodes/ep_*/` directories. There is no `_episodes/` folder under `episodes/`.
...
- `vacuum()` → rewrite `episodes.jsonl` excluding expired index rows (does not delete episode directories)
