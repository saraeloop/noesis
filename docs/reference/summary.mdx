...
Process metadata is present when the episode was started with `process=...` in
Python or `noesis run --process ...` in the CLI. It groups related episodes under
a stable operator-facing label.
...
<ResponseField name="process.id" type="string">
Opaque stable process identifier. It is derived from the resolved workspace
identity plus the optional process name.
</ResponseField>

<ResponseField name="process.name" type="string">
Human-readable process label. When no label is supplied, Noēsis derives one from
the workspace basename and process id prefix.
</ResponseField>

<ResponseField name="process.run_index" type="number | null">
Monotonic 1-based run number allocated within the process.
</ResponseField>

<ResponseField name="process.kind" type="string | null">
Execution style for the process. The current public run helpers record
`"oneshot"`.
</ResponseField>
