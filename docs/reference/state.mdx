...
### Troubleshooting projection mismatches

Use this runbook when `state.json` appears out of sync with `events.jsonl`.
...
# 1) Outcome status/summary/metrics should match projection evidence
assert projection.status == state["outcomes"]["status"]
assert projection.summary == state["outcomes"].get("summary")
assert projection.metrics == state["outcomes"].get("metrics", {})

# 2) Links should match the latest run.state_projection payload
assert projection.links == state["links"]
```

Interpretation:

- paused/non-terminal runs should only have `links.events` and `links.learn`
