...
When a run is started with a process label, `state.json` includes the stable
process identity and per-process run index:
...
Opaque process identifier derived from the resolved workspace identity plus the
optional process name.
</ResponseField>

<ResponseField name="process.name" type="string">
Operator-facing process label. If no explicit label was supplied, Noēsis derives
a workspace-based display name.
</ResponseField>

<ResponseField name="process.run_index" type="number">
Monotonic 1-based counter allocated by the process registry for this process.
</ResponseField>

<ResponseField name="process.kind" type="string">
Execution style recorded by the registry. The current runtime records `"oneshot"`
for `ns.run(...)`, `ns.solve(...)`, and `noesis run`.
</ResponseField>
