...
`ns.checkpoint(...)` writes a pointer, not a copy of the episode. Resume re-hashes:

- `events.jsonl` prefix (`event_offset` / `last_event_id`)
- `state.json` (`state_hash`)
- the current artifact set (`artifact_manifest_hash`)

Do not edit episode files while the run is paused. Adding, removing, or rewriting `manifest.json` after checkpoint also fails resume with `CheckpointConsistencyError`.

Paused runs stay unsealed (`final.json` / `manifest.json` are absent) until continuation or termination. Once `final.json` exists, `interrupt` / `checkpoint` / `resume` raise `RunSealedError`.
...
async def run_async_approval(task: str):
    ...
    if any(e.get("payload", {}).get("requires_approval") for e in events):
        ...
        if request.status == ApprovalStatus.APPROVED:
            # This starts a follow-up episode, not same-run continuation.
            # For pause/checkpoint/resume, use the pattern at the top of this page.
