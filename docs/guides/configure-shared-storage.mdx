...
By default, Noēsis writes episodes under `./.noesis/episodes/<episode-id>/` and process records under `./.noesis/processes/`. In a team or service setting, you usually want a **shared** location so everyone can see/replay the same traces.
...
<Check>
If you can answer “Where do episodes live in this environment?”, “Where do process records live?”, and “Who can read them?”, you’re ready to roll Noēsis out beyond your laptop.
</Check>

When `NOESIS_RUNS_DIR` points at an episodes directory (the default `.noesis/episodes`), process files are stored as a sibling: `.noesis/processes/`. Point shared mounts at the `.noesis` root if operators need both `noesis ps` and `noesis processes`.
