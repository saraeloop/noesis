...
Because `ns.solve()` is synchronous today, you must bridge the SDK's async stream into a synchronous call boundary. The Claude Agent SDK already owns the tool loop, so wrap the async query loop with `asyncio.run` (or an equivalent sync bridge).
...
def claude_agent_adapter(task: str) -> object:
