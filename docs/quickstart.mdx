...
.noesis/
  episodes/
    ep_01JH6Z2V9Q2K6Y6N0QZ7K2QW8C/   # episode id (ULID)
      ...
      final.json                    # seal (terminal runs only)
      manifest.json                 # SHA-256 + size ledger (terminal runs only)
...
Group related runs with `--process`:

```bash
noesis run "hello world" --process demo
noesis processes
noesis ps --process demo
```
