...
.noesis/index/
    episodes.jsonl                # best-effort EpisodeIndex (may be missing)
```

The index under `.noesis/index/` is written on a best-effort basis at the end of **sealed** runs and may be missing (for example, if indexing fails due to permissions). There is no `_episodes/` directory under `episodes/`.
