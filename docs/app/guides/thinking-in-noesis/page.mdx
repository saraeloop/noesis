...
Noēsis helps you model and supervise agent cognition with a tiny, stable API. You bring your graphs or tools (LangGraph, CrewAI, plain functions, and more); Noēsis wraps them in an observable cognitive loop:

- **Observe** → capture raw task and context
- **Interpret** (**Intuition**) → extract signals, risks, and intent
- **Plan** (**Intuition → Direction**) → propose or patch the next action (or veto it entirely)
- **Act** (**Direction**) → execute your adapter, graph, or tool
- **Reflect** (**Insight**) → evaluate outcomes, record reasons, capture metrics
...
- **Plan (Intuition → Direction)**: which tool to call next, optional patches, potential veto
...
```json
{
  ...
  "flags": {
    "intuition": true,
    "mode": "advisory",
    "direction": { "applied": 1, "vetoed": 0, "policy": "SqlGuardPolicy@1.0" }
  },
  "metrics": {
    "success": 1,
    "plan_count": 2,
