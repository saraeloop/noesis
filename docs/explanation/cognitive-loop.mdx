...
**What gets recorded**:
- Artifact reference to `learn.jsonl` (`learn_path`, `learn_schema`)
- Proposal IDs and counts (empty when no proposal was persisted)
- Whether any proposal was applied
- Causal link (`caused_by`) to the latest upstream event
...
Proposal bodies live in `learn.jsonl`, not inline in the learn event payload.
