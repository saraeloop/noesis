...
Episode IDs are `ep_<ULID>`:

- `ep_` prefix identifies it as an episode
- the ULID is monotonic and sortable (48-bit timestamp + 80-bit entropy)

```python
import noesis as ns

# Reproducible runs with the same seed under a deterministic clock
ep1 = ns.run("task", seed=42)
```

### Episode lifecycle

Runtime lifecycle states (schema `run_lifecycle_state_machine/1.0.0`):

```
running → interrupted → paused → resuming → running
running → success | failed | vetoed | cancelled | error
```

| Status | Meaning |
| --- | --- |
| `running` | Cognitive loop in progress |
| `interrupted` | `ns.interrupt(...)` evidence written |
| `paused` | Checkpoint written; run is unsealed |
| `resuming` | `ns.resume(...)` / `ns.resume_run(...)` in progress |
| `success` / `failed` / `vetoed` / `cancelled` / `error` | Terminal; `final.json` seals the run |

Paused runs do not write `final.json` until they later terminate.

## Processes

A **process** groups related episodes for operators. It is not a nested artifact folder.

```python
episode_id = ns.run("nightly triage", process="triage-bot")
```

- `process_id` is `sha256(workspace_identity|name)[:12]` (or just the workspace when no name is given)
- records live in `.noesis/processes/<process_id>.json`
- `summary.json` and `state.json` include a `process` block (`id`, `name`, `kind`, `run_index`)
- inspect with `noesis processes` and `noesis ps --process triage-bot`
...
.noesis/
  episodes/
    ep_<ULID>/             # episode directory
      summary.json         # metrics and outcomes
      state.json           # cognitive state
      events.jsonl         # timeline
      final.json           # seal (terminal runs)
      manifest.json        # integrity checksums (terminal runs)
      checkpoints/         # pause/resume pointers
      learn.jsonl          # learning signals (optional)
  processes/
    <process_id>.json      # operator identity for grouped runs
```
