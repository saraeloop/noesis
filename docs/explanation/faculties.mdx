...
- enforced veto with `governance_pause_on_veto=True`: `action_candidate → governance → run.interrupt → run.checkpoint` (no `act`)
...
### Veto runtime outcomes

When a veto is issued, the default runtime emits a terminate event:
...
_See `events.v1` for canonical payload shapes._
