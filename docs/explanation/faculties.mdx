...
Noēsis separates **faculties** (domain modules that run at specific hooks) from **phases** (the cognitive loop).
...
| Faculty | Role | Hook(s) | Artifact keys |
| --- | --- | --- | --- |
| **Intuition** | Policy guidance | interpret | `phase="intuition"` events |
| **Direction** | Plan mutations | direction | `phase="direction"` events |
| **Governance** | Pre-act audit | governance.pre_act | `phase="governance"` events |
| **Insight** | Metrics computation | finalize | `summary.json` metrics |
...
**Insight** computes metrics from episode traces during the finalize phase.
