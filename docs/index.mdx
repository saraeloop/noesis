...
Every run produces a clean artifact structure. Directories are **flat** (`ep_<ULID>/` under `.noesis/episodes/`). Group related runs with `process=` / `--process`; `ns.set(label=...)` is not a config key.
...
Paused runs stay unsealed (no `final.json` / `manifest.json`) until continuation.
